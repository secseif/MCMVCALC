 ...
 * @param {'SAC'|'Price'} system
 * @param {'mensal'|'anual'|'unico'|'personalizado'} extraType - todo mês, uma vez por ano (mês 12, 24, ...), um único aporte no mês 1, ou a cada `intervaloMeses` (ex.: FGTS a cada 24 meses)
 ...
 * @param {number} [intervaloMeses] - usado só quando extraType === 'personalizado': a cada quantos meses o aporte se repete (ex.: 24 para simular o saque do FGTS)
 */
  ...
  while (saldo > 0.005 && mes < maxMeses) {
    ...
    if (saldo > 0.005 && extraValue > 0) {
      ...
      if (aplicaEsteMes) {
        ...
        // Recalcular a parcela (em vez de encurtar o prazo) só faz sentido
        // matematicamente para um aporte ÚNICO: aí sim dá pra redistribuir
        // o saldo restante nos meses que faltam e chegar a uma parcela nova
        // e estável. Com aporte recorrente (mensal/anual), recalcular a
        // cada aporte enquanto o aporte continua entrando faria a parcela
        // desabar de forma instável nos últimos meses — então, para esses
        // casos, o abatimento sempre atua reduzindo o prazo.
        if (strategy === "parcela" && extraType === "unico") {
